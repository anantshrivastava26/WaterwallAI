...
## WaterwallAI integration quickstart

This project uses LM Studio's native v1 REST API (`/api/v1/chat`) over plain `httpx`. The `openai` Python client is no longer required.

1. Start LM Studio server and load your model.
2. Ensure `.env` has:
...
`LM_STUDIO_BASE_URL` is the bare host — the client appends `/api/v1/chat`. `LM_STUDIO_API_KEY` is sent as a `Bearer` token; LM Studio ignores it unless authentication is enabled.
...
4. Run end-to-end import + summary using your WhatsApp export:

```powershell
python scripts/phase1_import_and_summarize.py --chat-file "C:\Users\Asus\Downloads\WhatsApp Chat with familia🌻\WhatsApp Chat with familia🌻.txt" --api-base http://localhost:8000
```

The script uploads the chat to `/ingest/whatsapp`, infers the latest message date, and fetches `/summary/daily` for that date.
